...
- **A picture attached before signing in never arrived afterwards.** Staged image bytes are filed under the account they were staged for, and anything attached while signed out belongs to the signed-out one; the step that moves them across at sign-in existed but was never called, from either app. The upload then looked only under the new account, found nothing, and left the note pointing at a picture that could not load — a broken image no amount of re-syncing would repair. Signing in now adopts what the signed-out session staged. Signing in as a *different* account still does not: that wipes the device, and the previous session's pictures must not follow.
- **Signing out now clears cached picture bytes from memory.** Signing out already wiped notes and staged images from storage, but decrypted bytes cached for preview stayed in memory until the app was killed. They are dropped with everything else.
- **An image that could not be found is no longer kept forever as a broken one** — but only when it is genuinely gone. A store that cannot be read, as opposed to one with nothing in it, keeps the attachment and retries on the next sync, so a failing disk or a locked encrypted store cannot quietly delete a picture out of a note.

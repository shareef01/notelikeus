...
- A claimed attachment is never resurrected. `restore_note` skips rows carrying `delete_claimed_at` (the owner deleted it) or `purge_claimed_at` (a sweeper claimed it); attachments deleted only as a side effect of deleting the note carry neither and still come back.

...
| Cloudflare Pages `notelikeus-dev` | **Live staging (Supabase)** — https://notelikeus-dev.pages.dev/ (`VITE_ALLOW_SUPABASE_STAGING`; Hosting remains `notelike.web.app`). Production alias redeployed 2026-09-03 (`fd7dcfba`, source `900e976`) via `npm run deploy:staging-pages --branch=main`. Bundle calls `getRemoteNotesDataSource().uploadAllNotes` and includes `https://notelikeus-attachments.error-endpoint.workers.dev`. |
...
| Ops rehearsal RPC | **Done** (cloud continuation) — `npm run rehearse:staging-import` with a dedicated staging email user imported 1 note; `fetch_full_snapshot` returned 1. Does **not** replace the owner Google UI import. |
...
Cloud continuation (2026-09-03): workspace confirmed on merged `main` `92c750abac259b402c10a6c0df8fd420619b53ca`. Staging URLs respond (Pages 200, worker CORS for `https://notelikeus-dev.pages.dev`, Google authorize 302). Firebase Hosting `notelike.web.app` unchanged. **Do not** set `VITE_ALLOW_SUPABASE_PRODUCTION`.
...
5. Test-account migration rehearsal:
  - ~~Ops dump → backup JSON~~ **Done** — `npm run rehearse:staging-import` / `scripts/ops/fixtures/backup.rehearsal.example.json` (RPC path treats `apply_note_change` status as `applied`)
  - ~~Backup import uses the active remote (Supabase on Pages)~~ **Done** in `commitImportedNotes`. Pages production alias redeployed so https://notelikeus-dev.pages.dev/ serves that path.
  - ~~Ops RPC import against staging~~ **Done** from Cloud Agent (1 note / snapshot 1) without owner Google login.
  - **Owner:** sign in with Google on https://notelikeus-dev.pages.dev/, then Profile → Import backup with the example JSON. Repeat on Android/Desktop staging builds when convenient. This is the remaining owner-gated step.

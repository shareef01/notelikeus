 ...
 *
 * Every note action resolves through the local store then the remote data source, and the UI
 * invokes them from `void`-ed event handlers, so a rejection had nowhere to go: the note stayed
 * where it was while the handler's success toast (often with an Undo for an action that never
 * happened) was the only feedback.
